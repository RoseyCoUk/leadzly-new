...
- [ ] **NAP-03**: Existing `LocalBusiness` JSON-LD block updated with full `streetAddress`, `addressLocality` (Belfast), `postalCode` (BT5 5JE), `addressCountry` (GB), and `telephone` fields
...
- [ ] **EEAT-02**: `Person` JSON-LD block added to `<head>` for Allan Chan (name, jobTitle "Founder", url `https://leadzly.co`, sameAs LinkedIn URL)

### Additional Schema (SCHEMA)

- [ ] **SCHEMA-04**: `HowTo` JSON-LD block added to `<head>` mapping the 3-step "How It Works" process (step names, descriptions, and URLs matching the visible section content)
